...
function LoadData() {
  $.ajax({
    url: "./admin/view_comments2.php", // Menggunakan view_comments2.php
    type: "POST",
    dataType: "json",
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].parent_comment == 0) {
          var row = $(
            '<tr><td><b><img src="./gambar/avatar.jpeg" width="30px" height="30px" />' +
              data[i].student +
              " :<i> " +
              data[i].date +
          ...
          for (var r = 0; r < data.length; r++) {
            if (data[r].parent_comment == commentId) {
              var comments = $(
                '<tr><td style="padding-left:80px"><b><img src="./gambar/avatar.jpeg" width="30px" height="30px" />' +
                  data[r].student +
                  " :<i> " +
                  data[r].date +
...
$(document).ready(function () {
  $("#butsave").on("click", function () {
    ...
    if (name != "" && msg != "") {
      $.ajax({
        url: "./admin/save_comments2.php", // Menggunakan save_comments2.php
        type: "POST",
        data: {
          id: id,
          name: name,
